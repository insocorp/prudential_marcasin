...
* Comments.: conhecido como MATCH
* Programa alterada somente para gerar um arquivo solicitado em 12/09/2019
* Andamento: Milton,
* Existe uma rotina denominada MARCARSIN (creio que é este nome) que faz o match dos sinistros (no GED desta pasta tem um xml resultande desta rotina.
* Com base nela, preciso de um procedimento que liste os mesmos campos mas com este escopo :
* TP_PASTA = Cível com Sinistro VG
* Que não tenha conteúdo no campo NR_SINISTRO_IRB
* ambiente para validação : http://192.168.0.4/cgi-bin/isj/prudential_mig1/isj/
*/
...
function main(fcType , fcSinistro)
   ...
   local lcParam    := "wpbatch.ini",;
         lcVersao   := "Versao: carga_sinistro_i4pro_14/02/2020 - 11:52",; // deve-se alterar a data a cada envio
...
function procEnvioSinistro(fcPath,fcFile,fcSini)
   ...
   * setando as variaveis de ambiente                                                                                                             
   ...
   if llFlg
      aadd(laWhere,lcTabPasta  + ".TP_PASTA IN ('Cível com Sinistro VG')")
      aadd(laWhere,lcTabSeguro + ".NR_CONTROLE > 0")
      aadd(laWhere,"COALESCE(" + lcTabSeguro + ".NR_SINISTRO_IRB,'') = ''")
      ...
      *                     1        2        3      4        5    6 7 8 9 0 1
      lnError:= db_select(laCols,lcTabPasta,laJoin,laWhere,laOrder, , , , ,.f.,pnHOcor)
      if lnError == -1
...
* Return....: nil  
*/
function getValorReserva(fxVal,fxRet,fnRes,fnPasta,fnCont)
   local llFlg       := .T. // as logical
